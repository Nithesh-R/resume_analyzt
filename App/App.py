...
st.set_page_config(
   page_title="Resume Analyzt",
   page_icon='./Logo/recom.png',
)
...
def run():
    ...
    link = '<b>Developed by <a href="https://www.linkedin.com/in/nithesh-r/" style="text-decoration: none; color: #021659;">&copy; <u>Nithesh R</u></a></b>'
    ...
    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS CVV;"""
            ...
            if resume_data:
                ...
                for i in resume_data['skills']:
                    ...
                    #### Web development recommendation
                        ...
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                        text='Recommended skills genercdated from System',value=recommended_skills,key = '3')
                        st.markdown('''<h5 style='text-align: left; color: #1ed760;'>Adding this skills to resume will boost🚀 the chances of getting a Job💼</h5>''',unsafe_allow_html=True)
                        # course recommendation
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            ## Credentials 
            if ad_user == 'nithesh' and ad_password == 'adm@123':
                
                ### Fetch miscellaneous data from user_data(table) and convert it into dataframe
                cursor.execute('''SELECT ID, ip_add, resume_score, convert(Predicted_Field using utf8), convert(User_level using utf8), city, state, country from user_data_table''')
                datanalys = cursor.fetchall()
                plot_data = pd.DataFrame(datanalys, columns=['Idt', 'IP_add', 'resume_score', 'Predicted_Field', 'User_Level', 'City', 'State', 'Country'])
                
                ### Total Users Count with a Welcome Message
                values = plot_data.Idt.count()
                st.success("Welcome Nithesh ! Total %d " % values + " User's Have Used Our Tool : )")                
